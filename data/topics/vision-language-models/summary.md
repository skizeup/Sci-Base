...
Le diagramme suivant illustre l'architecture à deux branches des VLMs, où les représentations visuelles et textuelles sont projetées dans un espace commun :

```mermaid
flowchart TD
    A[Image] --> B[Encodeur visuel]
    C[Texte] --> D[Encodeur textuel]
    B --> E[Projection]
    D --> F[Projection]
    E --> G[Espace commun]
    F --> G
    G --> H{Tâche}
    H --> I[Description d image]
    H --> J[Recherche par texte]
    H --> K[Question visuelle]
```
